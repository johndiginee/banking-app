"""
    A simple banking system app using Python OOP
"""
class User:
    """
    This parent Class : User
        Holds details about a user
        Has a functionn to show user details
    """
...
class Bank(User):
    """
    This Child Class : Bank
        Stores details about the account balance
        Stores details about the amount
        Allows for deposits, withdraw, view balance, view total withdraws and total deposits
    """
    total_deposits = 0
    total_withdraws = 0
    ...
    def deposit(self):
        dp = float(input(f"{self.name.title()}, Please enter how much you'd like to deposit: "))
    ...
    def withdraws(self):
        wd = float(input(f"{self.name.title()}, Please enter how much you'd like to withdraw: "))
        if self.balance < wd:
            return f"You can't withdraw that amount"
        else:
    ...
    while True:
        ...
        if option_choice == 1:
            ...
            if option_choice == 5 and user_two != None:
                print(f"There have been {user.two.bank.total_deposits} deposits.\n")
        elif option_choice == 6:
            print("Thank for using Bank X.")
            return False
            break
        else:
...
while True:
    ...
    user_two = None
    new_user = input("Would you like to register a new customer? Type 'No' to create your bank. ")
    if new_user.lower() == 'yes':
        name = input("Enter the second customer's name: ")
        age = int(input("Enter the second customer's age: "))
